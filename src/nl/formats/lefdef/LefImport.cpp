...
#define  MSTREAM_P_SUPPORT(Type)                           \
  ...
  class LefParser {
    public:
      ...
      inline       PNLBox::Unit          fromUnitsMicrons         ( double ) const;
  ...
  inline       PNLBox::Unit         LefParser::getMinTerminalWidth      () const { return _minTerminalWidth; }
  inline       string            LefParser::getLibraryName           () const { return _libraryName; }
  inline       NLLibrary*          LefParser::getLibrary               ( bool create ) { if (not _library and create) createNLLibrary(); return _library; }
  ...
  inline       void              LefParser::setCoreSite              ( PNLBox::Unit x, PNLBox::Unit y ) { _coreSiteX=x; _coreSiteY=y; }
  inline       PNLBox::Unit         LefParser::getCoreSiteX             () { return _coreSiteX; }
  inline       PNLBox::Unit         LefParser::getCoreSiteY             () { return _coreSiteY; }
  ...
  int  LefParser::_siteCbk ( lefrCallbackType_e c, lefiSite* site, lefiUserData ud )
  {
    ...
    //   PNLBox::Unit lefSiteWidth  = PNLUnit::fromPhysical( site->sizeX(), PNLUnit::Micro );
    //   PNLBox::Unit lefSiteHeight = PNLUnit::fromPhysical( site->sizeY(), PNLUnit::Micro );
  ...
  int  LefParser::_macroForeignCbk ( lefrCallbackType_e c, const lefiMacroForeign* foreign, lefiUserData ud )
  {
    ...
    for ( PNLNet* net : cell->getNets() ) {
      ...
      if (bitNet->isVDD ()) parser->setGdsPower ( bitNet );
      if (bitNet->isGND()) parser->setGdsGround( bitNet );
  ...
  //cerr << "       @ _pinCbk: " << pin->name() << endl;
    ...
    if (pin->hasUse()) {
      ...
      if (lefUse == "SIGNAL") netType = PNLNet::Type::TypeEnum::Logical;
    //if (lefUse == "ANALOG") netType = PNLNet::Type::ANALOG;
      if (lefUse == "CLOCK" ) netType = PNLNet::Type::TypeEnum::Clock;
      if (lefUse == "POWER" ) netType = PNLNet::Type::TypeEnum::VDD;
      if (lefUse == "GROUND") netType = PNLNet::Type::TypeEnum::GND;
    }
    ...
    if (pin->hasDirection()) {
      ...
      if (lefDir == "INPUT"          ) term->setDirection( PNLNetComponent::Direction::Input       );
      if (lefDir == "OUTPUT"         ) term->setDirection( PNLNetComponent::Direction::Output      );
      if (lefDir == "OUTPUT TRISTATE") term->setDirection( PNLNetComponent::Direction::Tristate );
      if (lefDir == "INOUT"          ) term->setDirection( PNLNetComponent::Direction::InOut    );
    }
    ...
    //     if (geoms->itemType(igeom) == lefiGeomRectE) {
    //       lefiGeomRect* r          = geoms->getRect(igeom);
    //       PNLBox::Unit     w          = parser->fromUnitsMicrons(r->xh - r->xl);
    //       PNLBox::Unit     h          = parser->fromUnitsMicrons(r->yh - r->yl);
    //       Segment*      segment    = NULL;
  ...
  void  LefParser::_pinStdPostProcess ()
  {
    ...
    //         if (v) {
    //           PNLBox::Unit nearestX = gaugeMetal2->getTrackPosition( ab.getXMin()
    //                                                             , ab.getXMax()
    //                                                             , v->getX()
    //                                                             , Constant::Nearest );
    ...
    //                                                             , box.getXCenter()
    //                                                             , Constant::Nearest );
    //           PNLBox::Unit xmin = std::min( box.getXMin(), nearestX - gaugeMetal2->getViaWidth()/2 );
    //           PNLBox::Unit xmax = std::max( box.getXMax(), nearestX + gaugeMetal2->getViaWidth()/2 );
