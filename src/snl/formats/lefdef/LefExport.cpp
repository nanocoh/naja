...
#define  MSTREAM_P_SUPPORT(Type)                           \
  ...
  class LefDriver {
    public:
      ...
      static int                getUnits              ();
      static double             toLefUnits            ( PNLUnit::Unit );
      static PNLUnit::Unit          getSliceHeight        ();
      static PNLUnit::Unit          getPitchWidth         ();
                               ~LefDriver             ();
             int                write                 ();
    private:                                          
  ...
  PNLUnit::Unit          LefDriver::_sliceHeight = 0;
  PNLUnit::Unit          LefDriver::_pitchWidth  = 0;


         int                LefDriver::getUnits       () { return _units; }
         double             LefDriver::toLefUnits     ( PNLUnit::Unit u ) { return PNLUnit::toMicrons(u)/**getUnits()*/; }
         PNLUnit::Unit          LefDriver::getSliceHeight () { return _sliceHeight; }
  ...
  int  LefDriver::_versionCbk ( lefwCallbackType_e, lefiUserData udata )
  {
    ...
    comment << "For design <" << driver->getSNLLibraryName() << ">.";
  ...
  using std::cerr;
  using std::endl;
  using naja::SNL::SNLLibrary;
  using naja::PNL::PNLTransform;
